...
class creature {
  ...
  creature(int x, int y, int c, int c2uah, int c3ah, String n, int s, int g) {
    posx = x;
    posy = y;
    col = c;
    name = n;
  ...
  void reproduce() {
    ...
    creature offspring = new creature((int)(posx + random(-15,15)), (int)(posy + random(-15,15)), (int)((col+Math.random()-0.5)*1)%100, (int)((c2+Math.random()-0.5)*1)%100, (int)((c3+Math.random()-0.5)*1)%100, name,(int)bhealth,gen+1);
